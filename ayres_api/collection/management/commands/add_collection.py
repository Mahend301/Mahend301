...
class Command(BaseCommand):
    help = 'Populate the database with initial collections and NFTs from OnXRP'
    nfts_per_page = 500
    total_collection_in_csv = 'How much do you want.csv ?'
    onxrp_base_url = '?'
...
#belumfix
    ...
    def load_nfts(self):
        self.stdout.write('Fetching NFTs collections')
        for row in self.collections_from_csv:
    ...
    def add_nfts_to_database(self, onxrp_id: str, collection: Collection, nfts_response: list) -> None:
        ...
        for entry in nfts_response:
            ...
            nfts_to_atributes[entry['token_id']] = [
        ...
        for key, value in nfts_to_atributes.items():
    ...
    def collections_from_csv(self):
        self.stdout.write('Loading collection metadata from CSV')
