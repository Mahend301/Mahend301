...
class CollectionsAPIView(RetrieveAPIView):
    queryset = Collection.objects.prefetch_related('issuer')
...
class CollectionAPIView(RetrieveAPIView):
    quertset = Collection.objects.select_related('issuer')
    serializer_class = CollectionSerializer
    lookup_fields = ('issuer', 'taxon')
